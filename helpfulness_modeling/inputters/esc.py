...
try:
    from helpfulness_modeling.inputters.inputter_utils import BucketingDataLoader, DistributedBucketingDataLoader
except ModuleNotFoundError:
    from inputters.inputter_utils import  BucketingDataLoader, DistributedBucketingDataLoader
import logging
...
def featurize(bos, eos, inputs, max_input_length=512): # # # # 
    ...
    input_ids = situe_ids + context_ids + response_ids
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, **kwargs):
    ...
    fine_to_coarse = {
        0: 0, 1: 0, 2: 0, 3: 0, 4: 0, 13: 0, 14: 0, 5: 1, 6: 1, 7: 1, 8: 1, 9: 3, 10: 3, 11: 3, 12: 3, -100: -100
    }

    context = _norm(' '.join(data["context"]))
    context = process(context)
    dialog_id = data["dialog_id"]
    dialog_situa = process(dialog_id)
...
class DynamicBatchingLoader(object):
    ...
    def __init__(self, toker, batch_size, **kwargs):
        ...
        dev_index = json.load(open("dataset/esc/dev_index.json", 'r'))[0]
        with open("dataset/esc/esc_data.pkl", 'rb') as f:
