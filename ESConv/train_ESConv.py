...
# this is copied from https://github.com/thu-coai/Emotional-Support-Conversation/blob/main/codes_zcj/train.py
...
parser.add_argument("--train_batch_size", type=int, default=16, help="batch size now means per GPU per step")
...
#########################################################################
# _, model = build_model(checkpoint="/home/countrycelebration/code/Aligned-dPM-original/ESConv/DATA/strat.strat/2023-12-11190425.3e-05.16.1gpu/pytorch_model.bin", local_rank=args.local_rank, **names)
...
while True:
        ...
        if step % args.gradient_accumulation_steps == 0:
            ...
            if args.epoch_num is None and global_step % args.valid_step == 0:  # and epoch > 0:
                if args.local_rank == -1 or get_rank() == 0:
                    ...
                    if eval_ppl < best_metric:
                        # modification: save model with best eval ppl
                        print(f'Saving model with best eval ppl: {eval_ppl} at global step {global_step}')
                        torch.save(model.state_dict(), join(output_dir, f'pytorch_model.bin'))
                        toker.save_vocabulary(output_dir)
                        model.config.to_json_file(join(output_dir, f'config.json'))
                        best_metric = eval_ppl
                    ...
                    logger.info('current evaluation ppl: ' + str(eval_ppl))
                    model.train()
    ...
    if args.epoch_num is not None:
        if args.local_rank == -1 or get_rank() == 0:
            ...
            model.train()
            if eval_ppl < best_metric:
                print(f'Saving model with best eval ppl: {eval_ppl} at epoch {epoch}')
                torch.save(model.state_dict(), join(output_dir, 'pytorch_model.bin'))
                toker.save_vocabulary(output_dir)
                model.config.to_json_file(join(output_dir, f'config.json'))
                best_metric = eval_ppl
