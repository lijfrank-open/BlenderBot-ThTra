...
def featurize(
    ...
    decoder_input_idss = [bos] + labels[:-1]
...
def convert_data_to_inputs_sample(data, toker: PreTrainedTokenizer, **kwargs):
    # print("bbbbbbb")
    # print(len(PreTrainedTokenizer))
    process = lambda x: toker.convert_tokens_to_ids(toker.tokenize(x))
...
def get_sample_batch(infer_input_file, toker, **kwargs):
    ...
    for sample_id, line in tqdm.tqdm(enumerate(reader), total=len(reader), desc="sampling candidates", position=0, leave=True):
        data = json.loads(line)
        inputs = convert_data_to_inputs_sample(data, toker, **kwargs)
        tmp_features = convert_inputs_to_features(inputs, toker, **kwargs)
        for i in range(len(inputs)):
            features.append(tmp_features[i])
            ipt = inputs[i]
            situa.append(toker.decode(ipt['situa']))
            assert len(ipt['context']) == len(ipt['speaker'])
            contexts.append([toker.decode(utterance) for utterance in ipt['context'][-1:]])
            speakers.append(ipt['speaker'][-1:])
            references.append(toker.decode(ipt['response']))
            sample_ids.append(sample_id)

            if len(sample_ids) == infer_batch_size:
                yield prepare_infer_batch(features, toker), contexts, speakers, references, sample_ids, situa
                features = []
                sample_ids = []
                contexts = []
                speakers = []
