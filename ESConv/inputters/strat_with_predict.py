...
class InputFeatures(object):
    def __init__(
        ...
        self.decoder_input_idss = decoder_input_idss
        self.decoder_input_lengths = len(decoder_input_idss)
...
def featurize(
    ...
    labelss = (situation + [eos])[:max_decoder_input_length + 1]
    decoder_input_idss = [bos] + labelss[:-1]

    assert len(decoder_input_ids) == len(labels), decoder_input_ids[1:] == labels[:-1]
    assert len(decoder_input_idss) == len(labelss), decoder_input_idss[1:] == labelss[:-1]
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, **kwargs):
    ...
    context = []
    speaker = []  # added line
    context_with_predict = []
    situation = data['situation']
    situation = _norm(situation)
    ...
    for i in range(len(dialog)):
        ...
        if dialog[i]['speaker'] == 'sys':
            ...
            if i > 0:  # changed line
                ...
                # }
                # inputs.append(res)
                if i > 0 and i<len(dialog)-1 and dialog[i+1]['speaker'] == 'usr':
                    
                    prediction = _norm(dialog[i+1]['text'])
                    prediction = process(prediction)
                    context_with_predict  = context + [[process('Prediction')[0]] + prediction]
                else:
                    context_with_predict  = context 
                res = {
                    'situa': situation,
                    'context': context_with_predict.copy(),
                    'speaker': speaker.copy(),  # added line
                    'response': text,
                    'strat_id': strat_id,
                }
...
class FeatureDataset(Dataset):
    ...
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer, infer=False):
        ...
        if not infer:
            ...
            decoder_input_idss = pad_sequence([torch.tensor(f.decoder_input_idss, dtype=torch.long) for f in features],
                                             batch_first=True, padding_value=pad)
            labelss = pad_sequence([torch.tensor(f.labelss, dtype=torch.long) for f in features],
                                  batch_first=True, padding_value=-100)
            ...
            labels = None
            decoder_input_idss = torch.tensor([[f.decoder_input_idss[0]] for f in features], dtype=torch.long)
            labelss = None
        ...
        res = {
            ...
            'decoder_input_idss': decoder_input_idss,
            'labelss': labelss,
        }
