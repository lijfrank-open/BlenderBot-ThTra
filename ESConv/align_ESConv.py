...
def align_base_model(base_model, toker, train_dataloader, args, output_dir, logger=None):
    ...
    wandb.init(config=args, project="aligned-ESConv")
    last_update_time = 0
    while True:
        base_model.train()
...
def alignment(args):
    ...
    helpfulness_model_args = argparse.ArgumentParser()
    helpfulness_model_args_dict = vars(helpfulness_model_args)
    with open(os.path.join(args.helpfulness_model_dir, "args.json"), "r") as f:
        summary_dict = json.load(f)
        helpfulness_model_args_dict.update(summary_dict)
    args.mode = helpfulness_model_args.mode
    ...
    #########################################################################
    # sampling candidates and computing helpfulness scores
    #########################################################################
    ...
    helpfulness_mark = args.helpfulness_model_dir.split('/')[-1]
    if not os.path.exists(os.path.join(args.candidate_dir, f"helpfulness_score_{helpfulness_mark}.npy")):
        from helpfulness_modeling import compute_scores
        logger.info(f"computing helpfulness scores, helpfulness model: {args.mode}...")
        compute_scores(helpfulness_model_args, args, **args.dataloader_kwargs)

    #########################################################################
    # aligning base model with helpfulness scores
    #########################################################################
    align_dataloader = AlignmentDataLoader(
        ...
        helpfulness_mark=helpfulness_mark,
        **names
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--helpfulness_model_dir", type=str,
                        default="../helpfulness_modeling/output/esc_hem24-0327-2227_fold1")
    ...
    assert args.helpfulness_model_dir is not None, "``helpfulness_model_dir`` is invalid!"
